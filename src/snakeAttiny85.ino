 ...
 switch (snakeDirection) {
    case 0:
      for (int i = 0; i < 128; i++) {
        ...
        if (lastDigit == 0 || firstPart >= 15) {
          break;
        } else {
          holdingPos = firstPart;
          if(i == 0){
            firstPart++;
          }else{
    ...
    case 1:
      for (int i = 0; i < 128; i++) {
        ...
        secondPart = (snakeCords[i] >> 1) & 0x07; // Extract next 3 bits
        lastDigit = snakeCords[i] & 0x01;
        if (lastDigit == 0 || secondPart >= 7) {
          break;
        } else {
          holdingPos = secondPart;
          if(i == 0){
            secondPart++;
          }else{
  ...
  for (int i = 0; i < 128; i++) {
    ...
    oled.setFont(FONT6X8); oled.setCursor(40, 6); oled.print(secondPart);
    if (lastDigit == 0) {
      oled.setFont(FONT6X8); oled.setCursor(20, 6); oled.print(i);
      oled.setFont(FONT6X8); oled.setCursor(30, 6); oled.print(firstPart);
      break;
    }
    oled.setFont(FONT6X8); oled.setCursor(10, 6); oled.print(firstPart);
    oled.bitmap(firstPart * 8, secondPart, 8 + firstPart * 8, 1 + secondPart, snakeBody);
